import Emulator from './emulator.js';

let chip_test = new Uint8Array([
  ]);

window.onload = async () => {
  const response = await fetch("./target/wasm32-unknown-unknown/release/chipinho.wasm");

  const memory = new WebAssembly.Memory({
    initial: 100,
    maximum: 1000,
    shared: true,
  });

  const importObj = {
    Math: {
      random: () => Math.random(),
    },
    console: {
      log: () => console.log("cheguei aqui")
    },
    js: {
      mem: memory
    }
  };

  const {instance} = await WebAssembly.instantiateStreaming(response, importObj);
  const emulator = new Emulator(instance);
  console.log(emulator.load_program(chip_test));
  // console.log("opcode: ", emulator.get_opcode())

  document.getElementById('btn-tick').addEventListener('click', () => {
    console.log(emulator.get_opcode())
    let keypad = new Array(16);
    keypad.fill(0);
    keypad = new Uint8Array(keypad);
    console.log(emulator.program_counter)
    console.log(emulator.tick(keypad));
    console.log(emulator.program_counter)
  })
}
